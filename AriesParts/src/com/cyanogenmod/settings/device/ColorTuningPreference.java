...
public class ColorTuningPreference extends DialogPreference implements OnClickListener {
    ...
    class ColorSeekBar implements SeekBar.OnSeekBarChangeListener {
        ...
        public void resetDefault(String type, String path, int value) {
    ...
    public void onClick(View v) {
        switch(v.getId()) {
            case R.id.color_reset:
                for (int i = 0; i < SEEKBAR_ID.length; i++) {
                    mSeekBars[i].resetDefault("color", FILE_PATH[i], MAX_VALUE);
                }
                for (int i = 0; i < GAMMA_SEEKBAR_ID.length; i++) {
                    mSeekBars[SEEKBAR_ID.length + i].resetDefault("gamma", GAMMA_FILE_PATH[i], GAMMA_DEFAULT_VALUE);
                }
                break;
        }
    }
