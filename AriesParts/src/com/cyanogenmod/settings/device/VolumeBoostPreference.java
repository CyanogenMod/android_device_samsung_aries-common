    ...
    public void onClick(View v) {
        switch(v.getId()) {
            case R.id.volume_reset:
                for (int i = 0; i < BOOST_FILE_PATH.length; i++) {
                    Utils.writeValue(BOOST_FILE_PATH[i], String.valueOf(BOOST_DEFAULT_VALUE));
                    mSeekBars[i].resetDefault(BOOST_DEFAULT_VALUE);
                }
                for (int i = 0; i < MIC_FILE_PATH.length; i++) {
                    Utils.writeValue(MIC_FILE_PATH[i], String.valueOf(MIC_DEFAULT_VALUE));
                    mSeekBars[BOOST_FILE_PATH.length + i].resetDefault(MIC_DEFAULT_VALUE);
                }
                break;
        }
    }
